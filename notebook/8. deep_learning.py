...
Original file is located at
    https://colab.research.google.com/drive/1VKm00xHiVhdp1YY_jXBgZ9GjE3OtN0Aj
...
def train_full_bert(X_train, X_test, y_train, y_test, label_encoder, title=""):
    train_ds = TextDataset(X_train, y_train, tokenizer_full)
    test_ds  = TextDataset(X_test, y_test, tokenizer_full)
    ...
    print(f"\n===== HASIL BERT FULL FINE-TUNE — {title} =====")
    print(classification_report(
        y_test, preds, target_names=label_encoder.classes_, digits=4
    ))
...
X_train_d, X_test_d, y_train_d, y_test_d, enc_demo = build_dataset(df_demo)

pred_demo_fullbert = train_full_bert(
    X_train_d, X_test_d, y_train_d, y_test_d,
    enc_demo,
    title="Demographic"
)
...
X_train_c, X_test_c, y_train_c, y_test_c, enc_clin = build_dataset(df_clin)

pred_clin_fullbert = train_full_bert(
...
# Demo
acc_demo = accuracy_score(y_test_d, pred_demo_fullbert)
f1_demo = f1_score(y_test_d, pred_demo_fullbert, average="weighted")
summary_tl.append(["Demographic", "Full BERT", acc_demo, f1_demo])

# Clinical
acc_clin = accuracy_score(y_test_c, pred_clin_fullbert)
f1_clin = f1_score(y_test_c, pred_clin_fullbert, average="weighted")
summary_tl.append(["Clinical", "Full BERT", acc_clin, f1_clin])

df_summary_tl = pd.DataFrame(summary_tl, columns=["Dataset", "Model", "Accuracy", "F1"])
df_summary_tl
...
df_tl = pd.DataFrame([
    ["Demographic", "Full BERT", acc_demo, f1_demo],
    ["Clinical", "Full BERT", acc_clin, f1_clin]
...
sns.barplot(data=df_tl, x="Dataset", y="Accuracy", hue="Model", palette="Blues")
plt.title("Accuracy — Transfer Learning (Full BERT)", fontsize=14)
plt.ylim(0,1)
plt.ylabel("Accuracy")
plt.xlabel("Dataset")
plt.show()
...
plt.figure(figsize=(8,5))
sns.barplot(data=df_tl, x="Dataset", y="F1", hue="Model", palette="Greens")
plt.title("F1-Score — Transfer Learning (Full BERT)", fontsize=14)
plt.ylim(0,1)
plt.ylabel("F1-Score")
plt.xlabel("Dataset")
plt.show()
...
pivot_tl = df_tl.pivot(index="Model", columns="Dataset", values="Accuracy")

sns.heatmap(pivot_tl, annot=True, cmap="Purples", fmt=".3f")
plt.title("Heatmap Accuracy — Transfer Learning")
plt.show()

plt.figure(figsize=(6,4))
pivot_tl_f1 = df_tl.pivot(index="Model", columns="Dataset", values="F1")

sns.heatmap(pivot_tl_f1, annot=True, cmap="Greens", fmt=".3f")
plt.title("Heatmap F1-Score — Transfer Learning")
plt.show()

fig, ax = plt.subplots(1, 2, figsize=(14,5))

sns.barplot(data=df_tl, x="Dataset", y="Accuracy", hue="Model", palette="Blues", ax=ax[0])
ax[0].set_title("Accuracy — Full BERT")
ax[0].set_ylim(0,1)
ax[0].tick_params(axis='x')

sns.barplot(data=df_tl, x="Dataset", y="F1", hue="Model", palette="Greens", ax=ax[1])
ax[1].set_title("F1-Score — Full BERT")
ax[1].set_ylim(0,1)
...
print("=== RINGKASAN PERFORMA TRANSFER LEARNING (FULL BERT) ===")
df_tl.style.format({
    "Accuracy": "{:.4f}",
    "F1": "{:.4f}"
})
...
# ==== DEMOGRAPHIC ====
results.append(["Demographic", "RNN",
                accuracy_score(y_test_demo_arr, pred_rnn_demo),
                f1_score(y_test_demo_arr, pred_rnn_demo, average="weighted")])

results.append(["Demographic", "LSTM",
                accuracy_score(y_test_demo_arr, pred_lstm_demo),
                f1_score(y_test_demo_arr, pred_lstm_demo, average="weighted")])

results.append(["Demographic", "FT-LSTM",
                accuracy_score(y_test_demo_arr, pred_ft_demo),
                f1_score(y_test_demo_arr, pred_ft_demo, average="weighted")])

# LoRA demographic
results.append(["Demographic", "LoRA-BERT",
                accuracy_score(true_demo, preds_demo),
                f1_score(true_demo, preds_demo, average="weighted")])

# Full BERT demographic
results.append(["Demographic", "Full BERT",
                acc_demo, f1_demo])


# ==== CLINICAL ====
results.append(["Clinical", "RNN",
                accuracy_score(y_test_clin_arr, pred_rnn_clin),
                f1_score(y_test_clin_arr, pred_rnn_clin, average="weighted")])

results.append(["Clinical", "LSTM",
                accuracy_score(y_test_clin_arr, pred_lstm_clin),
                f1_score(y_test_clin_arr, pred_lstm_clin, average="weighted")])

results.append(["Clinical", "FT-LSTM",
                accuracy_score(y_test_clin_arr, pred_ft_clin),
                f1_score(y_test_clin_arr, pred_ft_clin, average="weighted")])

# LoRA clinical
results.append(["Clinical", "LoRA-BERT",
                accuracy_score(true_demo, preds_demo),
                f1_score(true_demo, preds_demo, average="weighted")])

# Full BERT clinical
results.append(["Clinical", "Full BERT",
                acc_clin, f1_clin])
...
plt.title("Perbandingan Accuracy Semua Model", fontsize=16)
plt.ylabel("Accuracy")
...
plt.title("Perbandingan F1-Score Semua Model", fontsize=16)
plt.ylabel("F1-Score")
plt.xticks(rotation=45)
plt.ylim(0,1)
plt.show()
...
sns.heatmap(pivot_acc, annot=True, cmap="Blues", fmt=".3f")
plt.title("Heatmap Accuracy")
plt.show()

plt.figure(figsize=(10,6))
sns.heatmap(pivot_f1, annot=True, cmap="Greens", fmt=".3f")
plt.title("Heatmap F1-Score")
plt.show()

fig, ax = plt.subplots(1, 2, figsize=(16,6))

sns.barplot(data=df_results, x="Model", y="Accuracy", hue="Dataset", ax=ax[0])
ax[0].set_title("Accuracy Comparison")
ax[0].set_ylim(0,1)
ax[0].tick_params(axis='x', rotation=45)

sns.barplot(data=df_results, x="Model", y="F1", hue="Dataset", ax=ax[1])
